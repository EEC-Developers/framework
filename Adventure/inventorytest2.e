...
MODULE 'dos/dos', 'adventure/items'
...
PROC main() HANDLE
  ...
  NEW everything.itemclass([lamp,torch,bag])
  ...
  REPEAT
    IF mystuff.has(light)
      WriteF('You see:\n')
      IF roomstuff.hasa(torch) THEN WriteF(' a torch\n')
      IF roomstuff.hasa(lamp) THEN WriteF(' a lamp\n')
      IF roomstuff.hasa(bag) THEN WriteF(' a bag\n')
    ELSE
